class Sneaker < ApplicationRecord
  ...
  validates :sneaker_name_en, presence: true, length: { maximum: 150 }, 
             format: { without: /[\p{Hiragana}\p{Katakana}\p{Han}]/, message: "はひらがなとカタカナと漢字では入力できません" }
  validates :sneaker_name_jp, presence: true, length: { maximum: 150 },
             format: { without: /[\p{Hiragana}\p{Han}a-zA-Z]/, message: "はひらがなと漢字、半角英字では入力できません" }
